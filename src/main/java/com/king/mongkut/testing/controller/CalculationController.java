...
public class CalculationController {
    ...
    @GetMapping("/multiply")
    public ResponseEntity<?> devide(@RequestParam String number1, @RequestParam String number2) {
        {
            int devide = calculationService.devide(number1, number2);
            if (devide != -1) {
                return ResponseEntity.ok(devide);
            } else {
                return ResponseEntity.badRequest().body("something went wrong");
            }
        }
